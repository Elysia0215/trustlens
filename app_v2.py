...
#### 🔝 핵심 개념 Top N
핵심 개념 허브에서 **자주 등장한 개념**을 막대로 보여줘요. 이 빈도·최근성은 **프로젝트 맵의 노드 크기·색**으로도 쓰여요.
""")
        with _g4c:
            st.markdown("#### 🤖 AI 브레인스토밍")
            st.markdown("""
...
""", unsafe_allow_html=True)
    ...
    <div style="font-weight:700; color:#1e293b; margin-top:6px;">{_v['title']}</div>
        ...
        _roadmap = [
            ("v3.6", "TF-IDF Highlight · 중요 개념 강조", "단순 빈도를 넘어 프로젝트 안에서 ‘특별히 중요한’ 개념을 TF-IDF로 가려내 맵·랭킹에 강조", "진행 예정"),
            ("v3.7", "Second Brain · 지식 아카이브 UX 개편", "‘저장소 → 두 번째 뇌’. 카드화·노트 상세(한 줄 핵심/핵심 개념/연결된 지식)·관련 메모 추천", "대기"),
            ("v3.8", "Alias Network · 별칭 시스템", "역전파 알고리즘→역전파처럼 같은 개념의 다른 표기를 비파괴적으로 연결", "대기"),
            ("v3.9", "Semantic Merge · AI 의미 병합", "신경망/신경망 구조/뉴럴 네트워크를 의미 유사도로 병합", "대기"),
...
""")
    ...
    <div style="background:#e2e8f0; border-radius:6px; height:9px; margin-top:3px;">
        ...
        st.info("👉 다음 전략: AI 분석 고도화보다 **TF-IDF 중요 개념 강조(v3.6) → 아카이브 UX·노트 상세(v3.7)** 로 ‘탐험 능력’을 끌어올리는 데 집중.")
        st.divider()
        ...
        _timeline = [
            ...
            ("2026-06", "v3.5 · Project Map", "관계가 보이다", "프로젝트 중심 작업·자료·개념·관계맵", "#7c3aed", True),
            ("예정", "v3.6 → v4.0", "Knowledge OS 완성", "TF-IDF 강조 → 두 번째 뇌 → 별칭 → 의미 병합 → 판단 패턴", "#7c3aed", False),
        ]
