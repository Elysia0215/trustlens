...
">
    ...
    <div style="font-size:1rem; opacity:0.9; line-height:1.6;">
        ...
        하나의 <strong>내 지식 세계</strong>로 만들어가요. <span style="opacity:0.8;">— 생각을 잇다 · 세계를 짓다</span>
    </div>
</div>
""", unsafe_allow_html=True)
    ...
    _quickstart = [
        ...
        ("3️⃣", "대시보드에서 '오늘의 세계'를 본다", "🏠 대시보드에서 메모·프로젝트·작업 수와 'AI가 발견한 연결'을 한눈에 봐요."),
        ("4️⃣", "🕸️ 지식그래프로 생각이 어떻게 연결됐는지 본다", "🌍 내 세계 → 지식 지도에서 메모↔개념↔태그가 이어진 내 생각 연결망을 눈으로 봐요."),
    ]
    ...
    for _ti, (_tem, _tnm, _tds) in enumerate(_terms):
        with (_tc1 if _ti % 2 == 0 else _tc2):
            st.markdown(
                ...
                unsafe_allow_html=True)
    st.caption(
        "🪐 한 줄 정리: JIUM에서 **당신은 메모·프로젝트·작업 3가지만** 다루면 돼요. "
        "개념·태그·관계·연구노트는 **AI가 알아서 뽑고 연결**해줘요."
    )
    st.divider()
    ...
    _menu_groups = [
        ...
        ("✍️", "기록하기", "#0ea5e9", "새 메모 · 데일리 노트",
         "생각을 적는 곳. 링크·글을 가져와 AI 초안으로도 만들어요."),
        ("🤖", "루미", "#f97316", "지식 AI · 브레인스토밍 · 패턴 분석",
         "AI가 내 기록을 분석하고 새 아이디어·연결을 제안해요."),
        ("⚙️", "설정", "#64748b", "설정 · 데이터·백업 · 가이드북 · 패치 노트",
         "환경 설정과 백업. 🧪 고급 모드를 켜면 개념·태그·관계 관리 메뉴가 나와요."),
    ]
    for _ge, _gn, _gc, _gitems, _gdesc in _menu_groups:
        st.markdown(
            f"<div style='border-left:5px solid {_gc};background:#f8fafc;border-radius:0 10px 10px 0;"
            f"padding:10px 16px;margin-bottom:8px;'>"
            f"<b style='color:{_gc};font-size:1.02rem;'>{_ge} {_gn}</b>"
            f"<span style='color:#94a3b8;font-size:0.85rem;'> &nbsp;·&nbsp; {_gitems}</span><br>"
            f"<span style='color:#475569;font-size:0.88rem;'>{_gdesc}</span></div>",
            unsafe_allow_html=True)
    st.caption("💡 처음엔 메뉴가 4그룹만 보여요. 전문가 기능(개념·태그·관계·분석)은 **설정 → 🧪 고급 모드**에서 켜요.")
    st.divider()
...
""", unsafe_allow_html=True)
    ...
    _CHANGELOG = [
        {
            "version": "v5.0",
            "codename": "JIUM — Personal Knowledge OS",
            "date": "2026-06-08",
            "title": "JIUM 대전환 — 저장에서 '생각 연결망'으로",
            "badge": "v5.0 완료",
            "problem": [
                "기능은 많은데 첫 사용자가 '뭘 눌러야 하지?'에서 멈춤(기능 발견 실패)",
                "메뉴가 엔티티(개발자) 기준 15~20개로 평면 나열돼 압도적",
                "가장 치명적: 작성한 메모가 재배포·재시작 때 사라지는 데이터 손실",
            ...
            "improvement": [
                ...
                "🕸️ 지식그래프를 대표 시각화로 승격 — 지식맵 진입 시 도구선택 없이 바로 그래프",
            ],
            "result": [
                "사용자가 보는 1차 객체는 메모·프로젝트·작업 3개로 단순화",
                "내 생각이 어떻게 연결되는지(메모↔개념↔태그↔메모)를 눈으로 봄",
                "홈 = 오늘의 세계 → 새 메모 → 루미 발견 → (펼침)성장·우주로 짧아짐",
            ],
            "features": [
                "🌐 전체 관계 그래프: 프로젝트→메모→(개념·태그·작업) + 메모↔메모 점선",
                "🧠 루미가 발견한 연결 카드(반복 개념 자동 감지) + 지식맵 입구",
                "🔗 링크·글 가져오기: 홈 토글 + 새 메모 폼 펼치기로 어디서나 접근",
                "관계 종류에 📝 메모 → 📝 메모(관련 메모/연구노트) 추가",
            ],
            "fixes": [
                "★ 데이터 손실 해결: 클라우드 읽기 실패 시 조용히 막히던 저장을 재시도(3회)+자동 해제로 복구",
                "저장 실패 시 ⚠️ 경고 배너로 '조용한 손실' 차단",
                "재시드 오버라이트 버그 제거(읽기 실패 시 옛 스냅샷으로 덮어쓰지 않음)",
                "홈 중첩 expander 크래시 수정(성장·우주·뇌지도를 토글로)",
                "대시보드 흰색 글자가 테마에 덮이던 문제(.jium-hero 클래스로 강제)",
            ],
            "infra": [
                "☁️ Supabase(jium_store) 영속 저장 — 재배포·재시작에도 데이터 생존",
                "legacy anon JWT 키 사용(새 publishable 키는 supabase 2.6.0 미지원)",
                "설정에 상설 🛠 개발자 진단 패널(연결 상태·저장/로드 비교·전체 로그 복사)",
            ],
        },
        {
            "version": "v4.3",
