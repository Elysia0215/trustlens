...
""", unsafe_allow_html=True)
    ...
    <div style="font-weight:700; color:#1e293b; margin-top:6px;">{_v['title']}</div>
        ...
        _roadmap = [
            ("v3.8", "Alias Network · 별칭 시스템", "역전파 알고리즘→역전파, BackPropagation→역전파처럼 같은 개념의 다른 표기를 비파괴적으로 연결 → 검색·맵·관련 메모 추천이 같은 개념군으로 인식", "진행 예정"),
            ("v3.9", "Settings Hub · 설정 통합", "흩어진 설정(테마·API 키·데이터·표시 옵션)을 한 곳으로 모아 관리", "대기"),
...
""")
    ...
    <div style="background:#e2e8f0; border-radius:6px; height:9px; margin-top:3px;">
        ...
        st.info("👉 다음 전략: **별칭(v3.8) → 설정 통합(v3.9) → 의미 병합(v4.0) → 사고 인바디(v4.x)** 로 ‘내 사고 구조를 읽어주는 지식 OS’를 향해 확장.")
        st.divider()
        ...
        _timeline = [
            ...
            ("예정", "v3.8 → v4.x", "Knowledge OS 완성", "별칭 → 설정 통합 → 의미 병합 → 사고 인바디(철학 프로파일)", "#7c3aed", False),
        ]
