...
def render_knowledge_map_page():
    ...
    tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8, tab9, tab10 = st.tabs(["📚 원노트 목차", "🧩 노션 보드", "🕸️ 태그 마인드맵", "🧠 지식 페이지", "🗂️ 개념 파인더", "🤖 AI 브레인스토밍", "🗺️ 프로젝트 지식맵", "🔗 관계형 지식맵", "📈 지식 성장", "🕰️ 타임라인"])
...
| 그룹 | 메뉴 | 역할 |
|------|------|------|
| 🏠 홈 | 홈 대시보드 / 통합 검색 / 새 메모·엔터티 | 시작점·수집·찾기 |
...
""", unsafe_allow_html=True)
    ...
    <div style="font-weight:700; color:#1e293b; margin-top:6px;">{_v['title']}</div>
        ...
        _roadmap = [
            ("v3.8", "Settings Hub · 설정 통합", "흩어진 설정을 ⚙️ 설정 허브로: 🎨 화면(라이트/다크·색상) · 🌌 세계관(기본·우주·숲·연구소) · 🤖 루미(말투·안내·자동 추천) · ✨ 인터페이스(애니메이션·토스트·자동 확장)", "진행 예정"),
            ("v3.9", "Alias Network · 별칭 시스템", "역전파 알고리즘→역전파, BackPropagation→역전파처럼 같은 개념의 다른 표기를 비파괴적으로 연결 → 검색·맵·관련 메모 추천이 같은 개념군으로 인식", "대기"),
            ("v4.0", "Semantic Merge · AI 의미 병합 + 판단 패턴", "의미 유사도 병합 + 홈을 ‘오늘 성장한 개념·최근 연결된 지식’ 중심으로, 판단 히스토리에서 사고 구조 발견", "대기"),
            ("v4.x", "Philosophy Profile · 사고 인바디", "메모·개념·작업·관계 데이터를 철학 축(안정↔도전·개인↔공동체·감정↔논리·탐색↔실행·이상↔현실)으로 환산해 ‘채연의 사고 구조’를 인바디처럼 리포트 (반복 개념·중요도 상승·변화 추이·AI 철학 상담 자동 리포트). 규칙 기반 MVP, API 불필요", "장기 차별화"),
        ]
...
""")
    ...
    <div style="background:#e2e8f0; border-radius:6px; height:9px; margin-top:3px;">
        ...
        st.info("👉 다음 전략: **설정 통합(v3.8) → 별칭(v3.9) → 의미 병합(v4.0) → 사고 인바디(v4.x)** 로 ‘내 사고 구조를 읽어주는 지식 OS’를 향해 확장.")
        st.divider()
        ...
        _timeline = [
            ...
            ("2026-06", "v3.7 · Second Brain", "다시 읽는 뇌", "아카이브 카드화 + 노트 상세 + 관련 메모 추천", "#7c3aed", True),
            ("예정", "v3.8 → v4.x", "Knowledge OS 완성", "설정 통합 → 별칭 → 의미 병합 → 사고 인바디(철학 프로파일)", "#7c3aed", False),
        ]
