...
""", unsafe_allow_html=True)
    ...
    <div style="font-weight:700; color:#1e293b; margin-top:6px;">{_v['title']}</div>
        ...
        _roadmap = [
            ("v3.9", "Settings Hub · 설정 통합", "흩어진 설정을 ⚙️ 설정 허브로: 🎨 화면(라이트/다크·색상) · 🌌 세계관(기본·우주·숲·연구소) · 🤖 루미(말투·안내·자동 추천) · ✨ 인터페이스(애니메이션·토스트·자동 확장)", "진행 예정"),
...
""")
    ...
    <div style="background:#e2e8f0; border-radius:6px; height:9px; margin-top:3px;">
        ...
        st.info("👉 다음 전략: **설정 통합(v3.9) → 의미 병합(v4.0) → 사고 인바디(v4.x)** 로 ‘내 사고 구조를 읽어주는 지식 OS’를 향해 확장.")
        st.divider()
        ...
        _timeline = [
            ...
            ("2026-06", "v3.8 · Alias Network", "개념이 묶이다", "역전파↔BackPropagation 비파괴 별칭 → 추천·맵·랭킹 합산", "#6d28d9", True),
            ("예정", "v3.9 → v4.x", "Knowledge OS 완성", "설정 통합 → 의미 병합 → 사고 인바디(철학 프로파일)", "#7c3aed", False),
        ]
