...
""", unsafe_allow_html=True)
    ...
    <div style="font-weight:700; color:#1e293b; margin-top:6px;">{_v['title']}</div>
        ...
        _roadmap = [
            ("v3.8", "Alias Network · 별칭 시스템", "역전파 알고리즘→역전파처럼 같은 개념의 다른 표기를 비파괴적으로 연결", "진행 예정"),
            ("v3.9", "Semantic Merge · AI 의미 병합", "신경망/신경망 구조/뉴럴 네트워크를 의미 유사도로 병합", "대기"),
            ("v4.0", "Knowledge OS · 판단 패턴 분석 + 살아있는 뇌", "홈을 ‘오늘 성장한 개념·최근 연결된 지식·연결 안 된 메모’ 중심으로, 판단 히스토리에서 사고 구조 발견", "대기"),
            ("v4.x", "Philosophy Profile · 사고 인바디", "개념·관계·최근성 데이터를 철학 축(안정↔도전·개인↔공동체·감정↔논리 등)으로 환산해 ‘채연의 사고 구조’ 리포트 생성 (규칙 기반 MVP)", "장기"),
        ]
...
""")
    ...
    <div style="background:#e2e8f0; border-radius:6px; height:9px; margin-top:3px;">
        ...
        st.info("👉 다음 전략: 입력 품질·탐험 경험을 끌어올린 뒤 **별칭(v3.8) → 의미 병합(v3.9) → 사고 인바디(v4.x)** 로 ‘살아있는 지식 OS’를 향해 확장.")
        st.divider()
        ...
        _timeline = [
            ...
            ("예정", "v3.8 → v4.x", "Knowledge OS 완성", "별칭 → 의미 병합 → 판단 패턴 → 사고 인바디", "#7c3aed", False),
        ]
